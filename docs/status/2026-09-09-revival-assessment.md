...
66 個配對全部通過 A3 的離散欄位精確比對（`exact` 無錯誤）；`derive_calibration_thresholds` 對這 66 對回傳 13 個門檻而沒有拋出上限錯誤。完整最小值／中位數／最大值在 [2026-09-09-a11-offline-pair-metrics.json](2026-09-09-a11-offline-pair-metrics.json)，腳本在 [2026-09-09-a11-offline-pair-metrics.py](2026-09-09-a11-offline-pair-metrics.py)。
...
- 離線診斷腳本與 JSON 附在 `docs/status/` 供核對；它不是 A11 receipt，也不是 Wave 0 PASS。
