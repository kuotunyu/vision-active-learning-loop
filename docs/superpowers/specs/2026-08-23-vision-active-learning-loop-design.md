...
**Status:** A11 independently calibrated same-host statistical-replay design amendment active
...
Three response designs were reviewed. A custom deterministic CUDA backward was rejected because it would introduce a new compiled numerical implementation, build toolchain, binary provenance surface, BF16 correctness proof, performance contract, and long-term maintenance burden into an active-learning portfolio whose research contribution is not CUDA kernel engineering. A CPU backward island was rejected because it would add nine GPU-to-CPU-to-GPU gradient boundaries per training step and materially change the canonical training topology and throughput. Directly replacing the A3 limits with values selected from A6 or A7 was rejected as post-hoc acceptance. A11 therefore adopts a **separately calibrated same-host empirical replay envelope with an independent holdout cohort**. This is a statistical reproducibility contract, not a deterministic-kernel claim.

The owner's delegated A11 design decision and this reviewed-protocol revision satisfy the later-review condition in Sections 5.2.9 and 19. Section 5.2.15 supersedes the A3 `math.isclose`, `1e-3`, and `0.99999` numerical acceptance limits only for a future A11 calibration and validation; it does not alter the A3 through A10 historical receipts or verdicts. The A3 warning exception, exact-comparison fields, and all non-numerical gates remain normative. No A6 or A7 value can satisfy A11 by implication.
...
every vector_relative_l2     <= 0.10
every cosine_defect          <= 0.005   # equivalent cosine >= 0.995
```
...
- reproducibility: `torch.use_deterministic_algorithms(True)`, cuDNN benchmark off, deterministic workspace settings, recorded RNG states, the exact Section 5.2.5 allowlisted grid-sample backward exception, and one verified Section 5.2.15 calibration-threshold receipt after `WAVE0_A11_PASS`.
...
1. the Section 5.2 model-contract receipt and Section 5.2.15 A11 aggregate receipt are `PASS` and hash-bound to the permitted lineage;
