...
**Status:** A7 CUDA grid-sample backward attribution design amendment owner-authorized / written specification review pending
**Milestone:** M0 complete for the original design; the A6 campaign is frozen at `WAVE0_A6_NORMATIVE_FAIL / WAVE1_FORBIDDEN`; A7 diagnostic design active
...
- [x] The recorded A6 numerical replay failure remains immutable evidence; A7 performs one bounded control/instrumented/isolated-VJP attribution campaign without changing thresholds, model identity, training claims, or Wave 0 status.
