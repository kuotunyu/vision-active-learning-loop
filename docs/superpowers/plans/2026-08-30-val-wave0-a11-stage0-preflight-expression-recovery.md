# Wave 0 A11 Stage 0 Preflight Expression Recovery Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Preserve the consumed v11 controller-preflight NO_GO, prove the parser-valid binding defect through a semantic RED/GREEN gate, and execute one fresh v12 Stage 0 raw-byte transport attempt before stopping.

**Architecture:** Two fresh plan-pinned PowerShell files live in one ignored v12 namespace. Their predecessor-absence check uses two separately parenthesized `Test-Path` command expressions joined by one Boolean `Or`; an AST gate distinguishes that shape from v11 before either source can run. The admitted v11 raw UTF-8/LF child and parent transport remains unchanged in role and remains single-use.
...
- Entry HEAD must be this committed plan, whose direct parent is design commit `15d34231f7f65f1be6f08eea5946eefd2b8949ba` and design blob is `545fe2d74e3209902f1733c180295d89b671ae86`.
- Exact author and committer: `kuotunyu <61350295+kuotunyu@users.noreply.github.com>`.
- Exact plan subject: `docs: plan A11 Stage 0 preflight expression recovery`.
- The plan commit changes only `docs/superpowers/plans/2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery.md`.
- The v12 namespace and both v12 sources must be absent at plan commit and execution entry.
...
- The Windows PowerShell executable is `C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe`; it is used once only for parser admission and never as the v12 child.
- Success stops at `ENTRYV12_STAGE0_TRANSPORT_PASS / DOWNSTREAM_NOT_AUTHORIZED`.

## File Map

| Path | Responsibility |
|---|---|
| `.superpowers/sdd/2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery/task-1-stage0-preflight-controller-v12.ps1` | Self-attests the v12/predecessor/runtime envelope, starts exactly one Stage 0 child, captures raw streams, verifies exact child bytes, and emits exact two-line raw parent output. |
| `.superpowers/sdd/2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery/task-1-stage0-v12.ps1` | Rechecks the read-only admission envelope and emits exactly one raw UTF-8/LF Stage 0 terminal. |

The successful v12 namespace contains exactly those two ordinary files and no directory child, marker, report, manifest, result, transcript, cache, bytecode, temporary, repair, or machine object.
...
SOURCE_IDENTITY|task-1-stage0-preflight-controller-v12.ps1|30109|50d3cad82e932b348b1fca433f62089795cdfe82804a3976ff686c767ecbf467
SOURCE_IDENTITY|task-1-stage0-v12.ps1|23048|6bff8750bd359adf0b21cf127720a58a20b2e0a02f4954fd1792a4683b5c6a9d
```
...
#### Frozen source: `task-1-stage0-preflight-controller-v12.ps1`
...
$CanonicalRoot = '<repo>'
$WorkspacePath = '<repo>\.worktrees\wave0-model-contract\.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery'
$ControllerPath = Join-Path $WorkspacePath 'task-1-stage0-preflight-controller-v12.ps1'
$Stage0Path = Join-Path $WorkspacePath 'task-1-stage0-v12.ps1'
...
$PlanPath = 'docs/superpowers/plans/2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery.md'
$DesignPath = 'docs/superpowers/specs/2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery-design.md'
...
$ExpectedDesign = '15d34231f7f65f1be6f08eea5946eefd2b8949ba'
$ExpectedDesignBlob = '545fe2d74e3209902f1733c180295d89b671ae86'
...
$ExpectedStage0Terminal = 'ENTRYV12_STAGE0_PASS|v11=preserved_no_go|v10=preserved_no_go|v9=preserved_no_go|v8=preserved_no_go|output=raw_utf8_lf|seed_files=2|v11_files=2|v10_files=3|v7_files=4|runtimes=2|providers=2|linked=clean|canonical=clean'
$TransportTerminal = 'ENTRYV12_RAW_BYTE_TRANSPORT_PASS|child_bytes=utf8_lf|parent_bytes=utf8_lf|stdin_bytes=0|starts=1|retries=0'
...
$V12ShadowPaths = @(
    '<workspace>\.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery',
    '<repo>\.worktrees\wave0-model-contract\CC_github部隊\vision-active-learning-loop\.worktrees\wave0-model-contract\.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery',
    '<repo>\CC_github部隊\vision-active-learning-loop\.worktrees\wave0-model-contract\.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery'
)

function Get-A11V12BytesSha256 {
...
function Assert-A11V12ParentChainOrdinary {
    param([Parameter(Mandatory)] [string] $Path)
...
function Assert-A11V12FileIdentity {
    ...
    Assert-A11V12ParentChainOrdinary -Path $Path
    if (-not [StringComparer]::Ordinal.Equals((Get-A11V12FileSha256 -Path $Path), $Sha256)) { throw 'file digest rejected' }
...
function Assert-A11V12SourceIdentity {
    param([string] $Path, [long] $ByteCount, [string] $Sha256)
    Assert-A11V12FileIdentity -Path $Path -ByteCount $ByteCount -Sha256 $Sha256 -LeafLinkType 'None'
    ...
    if ($RoundTrip.Length -ne $Bytes.Length -or (Get-A11V12BytesSha256 -Bytes $RoundTrip) -cne $Sha256) { throw 'source UTF-8 round trip rejected' }
    $Streams = @(Get-Item -LiteralPath $Path -Stream * -ErrorAction Stop)
    if ($Streams.Count -ne 1 -or [string]$Streams[0].Stream -cne ':$DATA') { throw 'source stream rejected' }
}

function Get-A11V12PlanSourceIdentity {
...
function Get-A11V12WorkspaceSnapshot {
    param([Parameter(Mandatory)] [string] $Path)
    ...
    foreach ($Item in @(Get-ChildItem -LiteralPath $Path -Force | Sort-Object Name)) {
        ...
        [void]$Records.Add("$($Item.Name)|$([long]$Item.Length)|$(Get-A11V12FileSha256 -Path $Item.FullName)")
    }
    [string]::Join("`n", $Records)
}

function Get-A11V12ExpectedV10MarkerText {
...
function Assert-A11V12PreservedEvidence {
    ...
    foreach ($Name in $ExpectedV11.Keys) { Assert-A11V12FileIdentity -Path (Join-Path $V11Workspace $Name) -ByteCount $ExpectedV11[$Name][0] -Sha256 $ExpectedV11[$Name][1] -LeafLinkType 'None' }
    ...
    foreach ($Name in $ExpectedV10.Keys) { Assert-A11V12FileIdentity -Path (Join-Path $V10Workspace $Name) -ByteCount $ExpectedV10[$Name][0] -Sha256 $ExpectedV10[$Name][1] -LeafLinkType 'None' }
    $ExpectedMarker = Get-A11V12ExpectedV10MarkerText
    ...
    foreach ($Name in $ExpectedExternalV9.Keys) { Assert-A11V12FileIdentity -Path (Join-Path $ExternalV9Workspace $Name) -ByteCount $ExpectedExternalV9[$Name][0] -Sha256 $ExpectedExternalV9[$Name][1] -LeafLinkType 'None' }
    ...
    foreach ($Path in $V12ShadowPaths) { if (Test-Path -LiteralPath $Path) { throw 'v12 shadow path exists' } }
    $PlanText = $Utf8.GetString([IO.File]::ReadAllBytes((Join-Path $RepositoryRoot $PlanPath)))
...
function Assert-A11V12Signature {
...
function Assert-A11V12RuntimeAndProviders {
    Assert-A11V12FileIdentity -Path $FrozenPwsh -ByteCount 301368 -Sha256 'db6dd81183fe57d22e03b911ec9a30a2fd7c40542e97743615355a6fb44f458f' -LeafLinkType 'None' -FileVersion '7.6.4.500' -ProductVersion '7.6.4 SHA: 929d27f4e66dcfba8f5f74ff03105705e483a27d+929d27f4e66dcfba8f5f74ff03105705e483a27d'
    Assert-A11V12FileIdentity -Path $FrozenWindowsPowerShell -ByteCount 454656 -Sha256 '7600ffe12da441fe89d035b13801e8e91d064bc544a27b19a5cf49f6ab8b18f5' -LeafLinkType 'HardLink' -FileVersion '10.0.26100.8875 (WinBuild.160101.0800)' -ProductVersion '10.0.26100.8875'
    Assert-A11V12FileIdentity -Path $PwshSecurityManifest -ByteCount 15463 -Sha256 'c3be79f92869f0f81050e16ce60736da7525f76d9c8784cf95bf4e83deddf62a' -LeafLinkType 'None'
    Assert-A11V12FileIdentity -Path $PwshSecurityDll -ByteCount 345952 -Sha256 '5e4d6fc14660e9e45a77e736580b9a770a12e3895d020c31c69baf0176d1bf90' -LeafLinkType 'None' -FileVersion '7.6.4.500' -ProductVersion '7.6.4 SHA: 929d27f4e66dcfba8f5f74ff03105705e483a27d+929d27f4e66dcfba8f5f74ff03105705e483a27d'
    Assert-A11V12FileIdentity -Path $WindowsSecurityManifest -ByteCount 776 -Sha256 'fa7150089e8a67a0aad27cd324d119b9778ccbead6242397780c5d5077246d30' -LeafLinkType 'HardLink'
    Assert-A11V12FileIdentity -Path $WindowsSecurityDll -ByteCount 93696 -Sha256 '9d24d2a9d3b5326a9bd8fcae8863fa5af32d37d7d0eee175600d7d4e89af8010' -LeafLinkType 'HardLink' -FileVersion '10.0.26100.1' -ProductVersion '10.0.26100.1'
}
...
try {
    ...
    $LocationPushed = $true
    Assert-A11V12LineageAndGit
    Assert-A11V12PreservedEvidence
    Assert-A11V12RuntimeAndProviders
    $ControllerIdentity = Get-A11V12PlanSourceIdentity -Name 'task-1-stage0-preflight-controller-v12.ps1'
    $Stage0Identity = Get-A11V12PlanSourceIdentity -Name 'task-1-stage0-v12.ps1'
    ...
    Assert-A11V12SourceIdentity -Path $ControllerPath -ByteCount $ControllerIdentity.byte_count -Sha256 $ControllerIdentity.sha256
    Assert-A11V12SourceIdentity -Path $Stage0Path -ByteCount $Stage0Identity.byte_count -Sha256 $Stage0Identity.sha256
    $Children = @(Get-ChildItem -LiteralPath $WorkspacePath -Force | Sort-Object Name)
    $ExpectedNames = @('task-1-stage0-preflight-controller-v12.ps1','task-1-stage0-v12.ps1') | Sort-Object
    if ($Children.Count -ne 2 -or @($Children | Where-Object PSIsContainer).Count -ne 0 -or [string]::Join("`n", @($Children.Name)) -cne [string]::Join("`n", $ExpectedNames)) { throw 'v12_inventory_rejected' }
    ...
    foreach ($Name in $ExpectedV7.Keys) { Assert-A11V12FileIdentity -Path (Join-Path $V7Workspace $Name) -ByteCount $ExpectedV7[$Name][0] -Sha256 $ExpectedV7[$Name][1] -LeafLinkType 'None' }
    $Before = Get-A11V12WorkspaceSnapshot -Path $WorkspacePath
    ...
    Assert-A11V12Signature -Signature (Get-AuthenticodeSignature -LiteralPath $FrozenPwsh) -Subject 'CN=Microsoft Corporation, O=Microsoft Corporation, L=Redmond, S=Washington, C=US' -Thumbprint 'AB172913A2960A224809EE8A0C371CD47A079B72'
    Assert-A11V12Signature -Signature (Get-AuthenticodeSignature -LiteralPath $FrozenWindowsPowerShell) -Subject 'CN=Microsoft Windows, O=Microsoft Corporation, L=Redmond, S=Washington, C=US' -Thumbprint 'DC91E564D5BC1E3A8E02D6A8508682ABEA8A2443'
    Assert-A11V12Signature -Signature (Get-AuthenticodeSignature -LiteralPath $PwshSecurityManifest) -Subject 'CN=Microsoft Corporation, O=Microsoft Corporation, L=Redmond, S=Washington, C=US' -Thumbprint 'AB172913A2960A224809EE8A0C371CD47A079B72'
    Assert-A11V12Signature -Signature (Get-AuthenticodeSignature -LiteralPath $PwshSecurityDll) -Subject 'CN=Microsoft Corporation, O=Microsoft Corporation, L=Redmond, S=Washington, C=US' -Thumbprint '1D77A9B9E8FE2075D9AD15123257FB90DB0DA4A1'
    Assert-A11V12Signature -Signature (Get-AuthenticodeSignature -LiteralPath $WindowsSecurityManifest) -Subject 'CN=Microsoft Windows, O=Microsoft Corporation, L=Redmond, S=Washington, C=US' -Thumbprint '71F53A26BB1625E466727183409A30D03D7923DF'
    Assert-A11V12Signature -Signature (Get-AuthenticodeSignature -LiteralPath $WindowsSecurityDll) -Subject 'CN=Microsoft Windows, O=Microsoft Corporation, L=Redmond, S=Washington, C=US' -Thumbprint '71F53A26BB1625E466727183409A30D03D7923DF'
    ...
    if ($StdoutBytes.Length -ne $ExpectedChildBytes.Length -or (Get-A11V12BytesSha256 -Bytes $StdoutBytes) -cne (Get-A11V12BytesSha256 -Bytes $ExpectedChildBytes) -or $Utf8.GetString($StdoutBytes) -cne ($ExpectedStage0Terminal + "`n")) { throw 'child_stdout_rejected' }
    if ((Get-A11V12WorkspaceSnapshot -Path $WorkspacePath) -cne $Before) { throw 'child_side_effect_rejected' }
...
if ($null -ne $Failure) {
    $FailureDigest = Get-A11V12BytesSha256 -Bytes $Utf8.GetBytes($Failure)
    $StdoutDigest = if ($StdoutComplete) { Get-A11V12BytesSha256 -Bytes $StdoutBytes } else { 'unavailable' }
    $StderrDigest = if ($StderrComplete) { Get-A11V12BytesSha256 -Bytes $StderrBytes } else { 'unavailable' }
    $ExitText = if ($ExitCodeAvailable) { [string]$ExitCode } else { 'unavailable' }
    $Diagnostic = "ENTRYV12_CONTROLLER_DIAGNOSTIC|phase=$FailurePhase|child_started=$($ChildStarted.ToString().ToLowerInvariant())|starts=$StartCount|exit=$ExitText|timed_out=$($TimedOut.ToString().ToLowerInvariant())|stdout_complete=$($StdoutComplete.ToString().ToLowerInvariant())|stdout_bytes=$($StdoutBytes.Length)|stdout_sha256=$StdoutDigest|stderr_complete=$($StderrComplete.ToString().ToLowerInvariant())|stderr_bytes=$($StderrBytes.Length)|stderr_sha256=$StderrDigest|exception_sha256=$FailureDigest`n"
...
#### Frozen source: `task-1-stage0-v12.ps1`
...
$WindowsSecurityManifest = 'C:\Windows\System32\WindowsPowerShell\v1.0\Modules\Microsoft.PowerShell.Security\Microsoft.PowerShell.Security.psd1'
$WindowsSecurityDll = 'C:\Windows\Microsoft.NET\assembly\GAC_MSIL\Microsoft.PowerShell.Security\v4.0_3.0.0.0__31bf3856ad364e35\Microsoft.PowerShell.Security.dll'
$Stage0Terminal = 'ENTRYV12_STAGE0_PASS|v11=preserved_no_go|v10=preserved_no_go|v9=preserved_no_go|v8=preserved_no_go|output=raw_utf8_lf|seed_files=2|v11_files=2|v10_files=3|v7_files=4|runtimes=2|providers=2|linked=clean|canonical=clean'
$Utf8 = [Text.UTF8Encoding]::new($false, $true)
$ExpectedV11 = [ordered]@{
    'task-1-stage0-raw-byte-controller-v11.ps1' = @([long]28071, 'e15b9115da4800a4833338a5b756e4cce54a3a1c38e3681705c762ef7f65ecb9')
    'task-1-stage0-v11.ps1' = @([long]21104, 'c800436da3365b77e4d5c9da456a1ae128b7cdf35a934f17bdd7efec74ac73aa')
...
function Get-A11V12BytesSha256 { param([byte[]] $Bytes) $Algorithm = [Security.Cryptography.SHA256]::Create(); try { [BitConverter]::ToString($Algorithm.ComputeHash($Bytes)).Replace('-', '').ToLowerInvariant() } finally { $Algorithm.Dispose() } }
function Get-A11V12FileSha256 { param([string] $Path) $Stream = [IO.File]::Open($Path,[IO.FileMode]::Open,[IO.FileAccess]::Read,[IO.FileShare]::Read); $Algorithm = [Security.Cryptography.SHA256]::Create(); try { [BitConverter]::ToString($Algorithm.ComputeHash($Stream)).Replace('-', '').ToLowerInvariant() } finally { $Algorithm.Dispose(); $Stream.Dispose() } }
function Assert-A11V12ParentChainOrdinary { param([string] $Path) $Cursor=[IO.Path]::GetDirectoryName([IO.Path]::GetFullPath($Path)); while(-not [string]::IsNullOrEmpty($Cursor)){ $Item=Get-Item -LiteralPath $Cursor -Force -ErrorAction Stop; if(($Item.Attributes-band[IO.FileAttributes]::ReparsePoint)-ne 0-or-not[string]::IsNullOrEmpty([string]$Item.LinkType)){throw 'linked parent rejected'}; $Parent=[IO.Directory]::GetParent($Cursor); if($null-eq$Parent){break}; $Cursor=$Parent.FullName } }
function Assert-A11V12FileIdentity { param([string]$Path,[long]$ByteCount,[string]$Sha256,[string]$LeafLinkType,[string]$FileVersion='',[string]$ProductVersion='') if(-not[IO.Path]::IsPathFullyQualified($Path)-or-not[StringComparer]::OrdinalIgnoreCase.Equals([IO.Path]::GetFullPath($Path),$Path)){throw 'file path rejected'}; $Item=Get-Item -LiteralPath $Path -Force -ErrorAction Stop; if($Item.PSIsContainer-or[long]$Item.Length-ne$ByteCount-or($Item.Attributes-band[IO.FileAttributes]::ReparsePoint)-ne 0){throw 'file identity rejected'}; $ActualLink=if([string]::IsNullOrEmpty([string]$Item.LinkType)){'None'}else{[string]$Item.LinkType}; if(-not[StringComparer]::Ordinal.Equals($ActualLink,$LeafLinkType)){throw 'file link rejected'}; Assert-A11V12ParentChainOrdinary $Path; if((Get-A11V12FileSha256 $Path)-cne$Sha256){throw 'file digest rejected'}; $Streams=@(Get-Item -LiteralPath $Path -Stream * -ErrorAction Stop); if($Streams.Count-ne 1-or[string]$Streams[0].Stream-cne':$DATA'){throw 'file stream rejected'}; if($FileVersion-and[string]$Item.VersionInfo.FileVersion-cne$FileVersion){throw 'file version rejected'}; if($ProductVersion-and[string]$Item.VersionInfo.ProductVersion-cne$ProductVersion){throw 'product version rejected'} }
function Assert-A11V12SourceIdentity { param([string]$Path,[long]$ByteCount,[string]$Sha256) Assert-A11V12FileIdentity $Path $ByteCount $Sha256 'None'; $Bytes=[IO.File]::ReadAllBytes($Path); if($Bytes.Length-eq 0-or($Bytes.Length-ge 3-and$Bytes[0]-eq 0xEF-and$Bytes[1]-eq 0xBB-and$Bytes[2]-eq 0xBF)-or$Bytes-contains 0x0D-or$Bytes[-1]-ne 0x0A-or($Bytes.Length-gt 1-and$Bytes[-2]-eq 0x0A)){throw 'source encoding rejected'}; $Round=$Utf8.GetBytes($Utf8.GetString($Bytes)); if($Round.Length-ne$Bytes.Length-or(Get-A11V12BytesSha256 $Round)-cne$Sha256){throw 'source roundtrip rejected'}; $Streams=@(Get-Item -LiteralPath $Path -Stream *); if($Streams.Count-ne 1-or[string]$Streams[0].Stream-cne ':$DATA'){throw 'source stream rejected'} }
function Get-A11V12PlanSourceIdentity { param([string]$Name) $PlanText=$Utf8.GetString([IO.File]::ReadAllBytes((Join-Path $RepositoryRoot $PlanPath))); $Matches=@($PlanText.Split([char]10)|Where-Object{$_.StartsWith("SOURCE_IDENTITY|$Name|",[StringComparison]::Ordinal)}); if($Matches.Count-ne 1){throw 'source record count rejected'}; $Parts=@($Matches[0].Split('|')); [long]$Count=0; if($Parts.Count-ne 4-or-not[long]::TryParse($Parts[2],[Globalization.NumberStyles]::None,[Globalization.CultureInfo]::InvariantCulture,[ref]$Count)-or$Parts[3]-notmatch'\A[0-9a-f]{64}\z'){throw 'source record rejected'}; [pscustomobject][ordered]@{byte_count=$Count;sha256=$Parts[3]} }
...
try {
    ...
    if(@(git status --porcelain=v1 --untracked-files=all).Count-ne 0-or@(git diff --cached --name-only).Count-ne 0-or@(git -C $CanonicalRoot status --porcelain=v1 --untracked-files=all).Count-ne 0){throw 'Git cleanliness rejected'}
    Assert-A11V12PreservedEvidence
    $ControllerIdentity=Get-A11V12PlanSourceIdentity 'task-1-stage0-preflight-controller-v12.ps1'; $Stage0Identity=Get-A11V12PlanSourceIdentity 'task-1-stage0-v12.ps1'
    if(-not[StringComparer]::OrdinalIgnoreCase.Equals([IO.Path]::GetFullPath($PSCommandPath),$Stage0Path)){throw 'Stage0 path rejected'}
    Assert-A11V12SourceIdentity $ControllerPath $ControllerIdentity.byte_count $ControllerIdentity.sha256; Assert-A11V12SourceIdentity $Stage0Path $Stage0Identity.byte_count $Stage0Identity.sha256
    $Children=@(Get-ChildItem -LiteralPath $WorkspacePath -Force|Sort-Object Name); $Names=@('task-1-stage0-preflight-controller-v12.ps1','task-1-stage0-v12.ps1')|Sort-Object; if($Children.Count-ne 2-or@($Children|Where-Object PSIsContainer).Count-ne 0-or[string]::Join("`n",@($Children.Name))-cne[string]::Join("`n",$Names)){throw 'v12 inventory rejected'}
    $V7Children=@(Get-ChildItem -LiteralPath $V7Workspace -Force); if($V7Children.Count-ne 4-or@($V7Children|Where-Object PSIsContainer).Count-ne 0){throw 'v7 inventory rejected'}; foreach($Name in $ExpectedV7.Keys){Assert-A11V12FileIdentity (Join-Path $V7Workspace $Name) $ExpectedV7[$Name][0] $ExpectedV7[$Name][1] 'None'}
    Assert-A11V12FileIdentity $FrozenPwsh 301368 'db6dd81183fe57d22e03b911ec9a30a2fd7c40542e97743615355a6fb44f458f' 'None' '7.6.4.500' '7.6.4 SHA: 929d27f4e66dcfba8f5f74ff03105705e483a27d+929d27f4e66dcfba8f5f74ff03105705e483a27d'
    Assert-A11V12FileIdentity $FrozenWindowsPowerShell 454656 '7600ffe12da441fe89d035b13801e8e91d064bc544a27b19a5cf49f6ab8b18f5' 'HardLink' '10.0.26100.8875 (WinBuild.160101.0800)' '10.0.26100.8875'
    Assert-A11V12FileIdentity $PwshSecurityManifest 15463 'c3be79f92869f0f81050e16ce60736da7525f76d9c8784cf95bf4e83deddf62a' 'None'; Assert-A11V12FileIdentity $PwshSecurityDll 345952 '5e4d6fc14660e9e45a77e736580b9a770a12e3895d020c31c69baf0176d1bf90' 'None' '7.6.4.500' '7.6.4 SHA: 929d27f4e66dcfba8f5f74ff03105705e483a27d+929d27f4e66dcfba8f5f74ff03105705e483a27d'
    Assert-A11V12FileIdentity $WindowsSecurityManifest 776 'fa7150089e8a67a0aad27cd324d119b9778ccbead6242397780c5d5077246d30' 'HardLink'; Assert-A11V12FileIdentity $WindowsSecurityDll 93696 '9d24d2a9d3b5326a9bd8fcae8863fa5af32d37d7d0eee175600d7d4e89af8010' 'HardLink' '10.0.26100.1' '10.0.26100.1'
    ...
    Assert-A11V12Signature (Get-AuthenticodeSignature -LiteralPath $FrozenPwsh) 'CN=Microsoft Corporation, O=Microsoft Corporation, L=Redmond, S=Washington, C=US' 'AB172913A2960A224809EE8A0C371CD47A079B72'; Assert-A11V12Signature (Get-AuthenticodeSignature -LiteralPath $FrozenWindowsPowerShell) 'CN=Microsoft Windows, O=Microsoft Corporation, L=Redmond, S=Washington, C=US' 'DC91E564D5BC1E3A8E02D6A8508682ABEA8A2443'; Assert-A11V12Signature (Get-AuthenticodeSignature -LiteralPath $PwshSecurityManifest) 'CN=Microsoft Corporation, O=Microsoft Corporation, L=Redmond, S=Washington, C=US' 'AB172913A2960A224809EE8A0C371CD47A079B72'; Assert-A11V12Signature (Get-AuthenticodeSignature -LiteralPath $PwshSecurityDll) 'CN=Microsoft Corporation, O=Microsoft Corporation, L=Redmond, S=Washington, C=US' '1D77A9B9E8FE2075D9AD15123257FB90DB0DA4A1'; Assert-A11V12Signature (Get-AuthenticodeSignature -LiteralPath $WindowsSecurityManifest) 'CN=Microsoft Windows, O=Microsoft Corporation, L=Redmond, S=Washington, C=US' '71F53A26BB1625E466727183409A30D03D7923DF'; Assert-A11V12Signature (Get-AuthenticodeSignature -LiteralPath $WindowsSecurityDll) 'CN=Microsoft Windows, O=Microsoft Corporation, L=Redmond, S=Washington, C=US' '71F53A26BB1625E466727183409A30D03D7923DF'
...
**Files:**
- Create: `.superpowers/sdd/2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery/task-1-stage0-preflight-controller-v12.ps1`
- Create: `.superpowers/sdd/2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery/task-1-stage0-v12.ps1`
- Read: `docs/superpowers/plans/2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery.md`
- Read: `docs/superpowers/specs/2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery-design.md`

**Interfaces:**
- Consumes: exact committed design/plan lineage, absent v12 namespace, exact preserved v11 two-file NO_GO, inherited predecessor records, runtime/provider identities, and clean linked/canonical worktrees.
- Produces: exactly two immutable v12 source files whose bytes equal the frozen plan blocks.

- [ ] **Step 1: Run the complete read-only entry gate**

From the linked repository, require exact plan parent, design/plan blobs, author, committer, subject, one changed plan path, branch, linked-worktree isolation, non-submodule state, empty staging, clean linked/canonical worktrees, absent v12 namespace and all three shadow paths, exact v11 two-file identities and incident record, exact inherited v10/v9/v7 records, required v8/v6/intended-v9 absences, and exact six runtime/provider identities and signatures.
...
if($Head-notmatch'\A[0-9a-f]{40}\z'-or(git rev-list --parents -n 1 HEAD)-cne"$Head 15d34231f7f65f1be6f08eea5946eefd2b8949ba"){throw 'plan parent rejected'}
if((git rev-parse "HEAD:$DesignPath")-cne'545fe2d74e3209902f1733c180295d89b671ae86'){throw 'design blob rejected'}
if((git hash-object -- $PlanPath)-cne(git rev-parse "HEAD:$PlanPath")){throw 'working plan blob rejected'}
if((git show -s --format='%an|%ae|%cn|%ce|%s' HEAD)-cne'kuotunyu|61350295+kuotunyu@users.noreply.github.com|kuotunyu|61350295+kuotunyu@users.noreply.github.com|docs: plan A11 Stage 0 preflight expression recovery'){throw 'plan identity rejected'}
$Changed=@(git diff-tree --no-commit-id --name-only -r HEAD)
...
'ENTRYV12_MATERIALIZATION_ENTRY_PASS|v11_files=2|v12=absent|runtimes=2|providers=2|linked=clean|canonical=clean'
```

Any difference stops without materialization as `ENTRYV12_MATERIALIZATION_UNPROVABLE / NO_GO`.
...
'ENTRYV12_SOURCE_BLOCK_ADMISSION_PASS|files=2|parser=7.6.4|errors=0'
```
...
```text
CC_github部隊/vision-active-learning-loop/.worktrees/wave0-model-contract/.superpowers/sdd/2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery/task-1-stage0-preflight-controller-v12.ps1
CC_github部隊/vision-active-learning-loop/.worktrees/wave0-model-contract/.superpowers/sdd/2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery/task-1-stage0-v12.ps1
```
...
- [ ] **Step 4: Verify exact materialization and preservation**

Require exactly two ordinary v12 files and no directory child; exact plan-pinned byte counts and SHA-256; strict UTF-8/no-BOM/LF/single-final-LF; canonical containment; ordinary parents; no link/reparse point; one default data stream; all v12 shadows absent; exact v11/v10/v9/v7 evidence unchanged; required v8/v6/intended-v9 absences; exact runtime/providers/signatures; unchanged HEAD/branch; empty staging; and clean linked/canonical worktrees.

```powershell
$Workspace='<repo>\.worktrees\wave0-model-contract\.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery'
$Children=@(Get-ChildItem -LiteralPath $Workspace -Force|Sort-Object Name)
if($Children.Count-ne2-or@($Children|Where-Object PSIsContainer).Count-ne0){throw 'v12 inventory rejected'}
$Expected=[ordered]@{'task-1-stage0-preflight-controller-v12.ps1'=@(30109,'50d3cad82e932b348b1fca433f62089795cdfe82804a3976ff686c767ecbf467');'task-1-stage0-v12.ps1'=@(23048,'6bff8750bd359adf0b21cf127720a58a20b2e0a02f4954fd1792a4683b5c6a9d')}
foreach($Name in $Expected.Keys){$Path=Join-Path $Workspace $Name;$Item=Get-Item -LiteralPath $Path -Force;$Bytes=[IO.File]::ReadAllBytes($Path);$Streams=@(Get-Item -LiteralPath $Path -Stream *);$Sha=(Get-FileHash -LiteralPath $Path -Algorithm SHA256).Hash.ToLowerInvariant();if($Item.PSIsContainer-or$Item.Length-ne[long]$Expected[$Name][0]-or$Sha-cne[string]$Expected[$Name][1]-or($Item.Attributes-band[IO.FileAttributes]::ReparsePoint)-ne0-or-not[string]::IsNullOrEmpty([string]$Item.LinkType)-or$Streams.Count-ne1-or[string]$Streams[0].Stream-cne':$DATA'-or$Bytes-contains0x0D-or$Bytes[-1]-ne0x0A-or($Bytes.Length-gt1-and$Bytes[-2]-eq0x0A)){throw "v12 source rejected: $Name"}}
$Shadow=@('<workspace>\.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery','<repo>\.worktrees\wave0-model-contract\CC_github部隊\vision-active-learning-loop\.worktrees\wave0-model-contract\.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery','<repo>\CC_github部隊\vision-active-learning-loop\.worktrees\wave0-model-contract\.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery');foreach($Path in $Shadow){if(Test-Path -LiteralPath $Path){throw 'v12 shadow rejected'}}
...
'ENTRYV12_MATERIALIZATION_PASS|files=2|directories=0|shadows=0|writes=1|retries=0'
```

### Task 2: Behavioral RED and semantic/static GREEN admission
...
- Test: `.superpowers/sdd/2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery/task-1-stage0-preflight-controller-v12.ps1`
- Test: `.superpowers/sdd/2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery/task-1-stage0-v12.ps1`

**Interfaces:**
- Consumes: exact Task 1 inventory and immutable v11 runtime failure.
- Produces: one v11 semantic RED, dual-parser admission, and one v12 semantic/raw-output GREEN without invoking either v12 source.

- [ ] **Step 1: Run the immutable v11 semantic RED**

Read v11 controller bytes as data, require its exact identity and line 171, parse only its AST, and require zero parser errors, one condition `CommandAst`, two `LiteralPath` parameters, zero `Or` binary nodes, and zero separating parenthesized nodes. Hash the exact frozen parameter-binding message and require the diagnostic digest. Do not invoke, import, or dot-source v11.

```powershell
$Path='<repo>\.worktrees\wave0-model-contract\.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-raw-byte-output-recovery\task-1-stage0-raw-byte-controller-v11.ps1'
$Bytes=[IO.File]::ReadAllBytes($Path)
if($Bytes.Length-ne28071-or(Get-FileHash -LiteralPath $Path -Algorithm SHA256).Hash.ToLowerInvariant()-cne'e15b9115da4800a4833338a5b756e4cce54a3a1c38e3681705c762ef7f65ecb9'){throw 'v11 identity rejected'}
$Tokens=$null;$Errors=$null;$Ast=[Management.Automation.Language.Parser]::ParseFile($Path,[ref]$Tokens,[ref]$Errors)
$If=@($Ast.FindAll({param($N)$N-is[Management.Automation.Language.IfStatementAst]-and$N.Extent.StartLineNumber-eq171},$true))
if(@($Errors).Count-ne0-or$If.Count-ne1){throw 'v11 RED parser surface rejected'}
$Commands=@($If[0].FindAll({param($N)$N-is[Management.Automation.Language.CommandAst]},$true))
$Literal=@($Commands|ForEach-Object{$_.CommandElements}|Where-Object{$_-is[Management.Automation.Language.CommandParameterAst]-and$_.ParameterName-ceq'LiteralPath'})
$Or=@($If[0].FindAll({param($N)$N-is[Management.Automation.Language.BinaryExpressionAst]-and$N.Operator-eq[Management.Automation.Language.TokenKind]::Or},$true))
$Paren=@($If[0].FindAll({param($N)$N-is[Management.Automation.Language.ParenExpressionAst]},$true))
if($Commands.Count-ne1-or$Literal.Count-ne2-or$Or.Count-ne0-or$Paren.Count-ne0){throw 'v11 RED AST rejected'}
$Message='Cannot bind parameter because parameter ''LiteralPath'' is specified more than once. To provide multiple values to parameters that can accept multiple values, use the array syntax. For example, "-parameter value1,value2,value3".'
$Utf8=[Text.UTF8Encoding]::new($false,$true);$A=[Security.Cryptography.SHA256]::Create();try{$MessageSha=[BitConverter]::ToString($A.ComputeHash($Utf8.GetBytes($Message))).Replace('-','').ToLowerInvariant()}finally{$A.Dispose()}
if($MessageSha-cne'7b998330838b1cc066caf7f29ecd0cfffdc3113eb270bbb8175fb12197525191'){throw 'v11 diagnostic digest rejected'}
'ENTRYV12_PREFLIGHT_EXPRESSION_RED_PASS|v11_commands=1|literalpath=2|binary_or=0|child_starts=0|v11_retries=0'
```

- [ ] **Step 2: Run PowerShell 7 and Windows PowerShell 5.1 parser admission**

PowerShell 7 parses both v12 files in the controller shell. Then this command
starts the exact Windows PowerShell executable once and uses only
`Parser.ParseFile` on the same canonical files:

```powershell
$Workspace='<repo>\.worktrees\wave0-model-contract\.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery'
foreach($Name in @('task-1-stage0-preflight-controller-v12.ps1','task-1-stage0-v12.ps1')){$Tokens=$null;$Errors=$null;[void][Management.Automation.Language.Parser]::ParseFile((Join-Path $Workspace $Name),[ref]$Tokens,[ref]$Errors);if(@($Errors).Count-ne0){throw 'PowerShell 7 parser rejected'}}
'ENTRYV12_PARSER_PASS|runtime=powershell_core_7_6_4|files=2|errors=0'
$env:A11V12_WORKSPACE=$Workspace
try{
    & 'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe' -NoLogo -NoProfile -NonInteractive -Command '& { $ErrorActionPreference="Stop";if(-not[StringComparer]::OrdinalIgnoreCase.Equals([IO.Path]::GetFullPath([Diagnostics.Process]::GetCurrentProcess().MainModule.FileName),"C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe")-or$PSVersionTable.PSVersion.Major-ne5-or$PSVersionTable.PSVersion.Minor-ne1){throw "parser process rejected"};foreach($Name in @("task-1-stage0-preflight-controller-v12.ps1","task-1-stage0-v12.ps1")){$Tokens=$null;$Errors=$null;[void][Management.Automation.Language.Parser]::ParseFile((Join-Path $env:A11V12_WORKSPACE $Name),[ref]$Tokens,[ref]$Errors);if(@($Errors).Count-ne0){throw "Windows PowerShell parser rejected"}};"ENTRYV12_PARSER_PASS|runtime=windows_powershell_5_1|files=2|errors=0" }'
    if($LASTEXITCODE-ne0){throw 'Windows PowerShell parser process rejected'}
}
finally{Remove-Item Env:A11V12_WORKSPACE -ErrorAction SilentlyContinue}
```

Any parser process uncertainty is `ENTRYV12_PREFLIGHT_CONTROLLER_UNPROVABLE / NO_GO`; do not start it again.

- [ ] **Step 3: Run the semantic mutation and complete static GREEN**

Parse the exact frozen v11 condition literal and corrected v12 literal in memory without executing either. Require both parser error counts zero; v11 must have one command/two duplicate `LiteralPath`/zero `Or`/zero parentheses, while v12 must have two commands/one `LiteralPath` each/one `Or`/two parentheses. Then inspect both canonical v12 source ASTs and require the same corrected shape at every predecessor-absence condition.
...
'ENTRYV12_PREFLIGHT_EXPRESSION_GREEN_PASS|commands=2|literalpath_each=1|binary_or=1|paren=2|starts=0'
$Workspace='<repo>\.worktrees\wave0-model-contract\.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery';$Controller=Join-Path $Workspace 'task-1-stage0-preflight-controller-v12.ps1';$Stage0=Join-Path $Workspace 'task-1-stage0-v12.ps1'
...
$Utf8=[Text.UTF8Encoding]::new($false,$true);$Child=$Utf8.GetBytes('ENTRYV12_STAGE0_PASS|v11=preserved_no_go|v10=preserved_no_go|v9=preserved_no_go|v8=preserved_no_go|output=raw_utf8_lf|seed_files=2|v11_files=2|v10_files=3|v7_files=4|runtimes=2|providers=2|linked=clean|canonical=clean'+"`n");$Parent=$Utf8.GetBytes($Utf8.GetString($Child)+'ENTRYV12_RAW_BYTE_TRANSPORT_PASS|child_bytes=utf8_lf|parent_bytes=utf8_lf|stdin_bytes=0|starts=1|retries=0'+"`n")
...
if($Child.Length-ne218-or(Get-StaticSha $Child)-cne'14e0399a8b118beabc96a9fb9d95f119a08d04f8b051c9adbbcc388392afabc1'-or$Parent.Length-ne325-or(Get-StaticSha $Parent)-cne'd330bed4652d52295271040cda4402978eef125b694998566bd97949f22c17a9'){throw 'raw byte contract rejected'}
'ENTRYV12_OUTPUT_CONTRACT_GREEN_PASS|preflight=semantic_or|child=raw_utf8_lf|parent=raw_utf8_lf|task_results=suppressed|parsers=2|starts=0'
```

Immediately after GREEN, run this independent preservation check. Any
difference preserves both v12 files and stops before Task 3.

```powershell
$RepositoryRoot='<repo>\.worktrees\wave0-model-contract';$CanonicalRoot='<repo>';$V12=Join-Path $RepositoryRoot '.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery';$V11=Join-Path $RepositoryRoot '.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-raw-byte-output-recovery'
$Expected12=[ordered]@{'task-1-stage0-preflight-controller-v12.ps1'=@(30109,'50d3cad82e932b348b1fca433f62089795cdfe82804a3976ff686c767ecbf467');'task-1-stage0-v12.ps1'=@(23048,'6bff8750bd359adf0b21cf127720a58a20b2e0a02f4954fd1792a4683b5c6a9d')};$Expected11=[ordered]@{'task-1-stage0-raw-byte-controller-v11.ps1'=@(28071,'e15b9115da4800a4833338a5b756e4cce54a3a1c38e3681705c762ef7f65ecb9');'task-1-stage0-v11.ps1'=@(21104,'c800436da3365b77e4d5c9da456a1ae128b7cdf35a934f17bdd7efec74ac73aa')}
foreach($Pair in @(@($V12,$Expected12),@($V11,$Expected11))){$Root=[string]$Pair[0];$Expected=[Collections.IDictionary]$Pair[1];$Items=@(Get-ChildItem -LiteralPath $Root -Force);if($Items.Count-ne$Expected.Count-or@($Items|Where-Object PSIsContainer).Count-ne0){throw 'GREEN preservation inventory rejected'};foreach($Name in $Expected.Keys){$Path=Join-Path $Root $Name;if((Get-Item -LiteralPath $Path).Length-ne[long]$Expected[$Name][0]-or(Get-FileHash -LiteralPath $Path -Algorithm SHA256).Hash.ToLowerInvariant()-cne[string]$Expected[$Name][1]){throw 'GREEN preservation identity rejected'}}}
...
'ENTRYV12_PREINVOCATION_PRESERVATION_PASS|v12_files=2|v11_files=2|starts=0|linked=clean|canonical=clean'
```

### Task 3: Single-use Stage 0 observation and final closure

**Files:**
- Execute once: `.superpowers/sdd/2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery/task-1-stage0-preflight-controller-v12.ps1`
- Read: both v12 files, v11/v10/predecessor evidence, design, and plan

**Interfaces:**
- Consumes: exact Task 1 materialization and Task 2 RED/GREEN terminals.
- Produces: exact two-line v12 controller stdout plus final closed milestone, or the first preserved NO_GO.
...
if($Child.Length-ne218-or(Get-BoundarySha $Child)-cne'14e0399a8b118beabc96a9fb9d95f119a08d04f8b051c9adbbcc388392afabc1'-or$Parent.Length-ne325-or(Get-BoundarySha $Parent)-cne'd330bed4652d52295271040cda4402978eef125b694998566bd97949f22c17a9'){throw 'boundary byte contract rejected'}
'ENTRYV12_INVOCATION_BOUNDARY|exe=C:\Users\<user>\.cache\codex-runtimes\codex-primary-runtime\dependencies\native\powershell\pwsh.exe|exe_bytes=301368|exe_sha256=db6dd81183fe57d22e03b911ec9a30a2fd7c40542e97743615355a6fb44f458f|controller=.\.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery\task-1-stage0-preflight-controller-v12.ps1|controller_bytes=30109|controller_sha256=50d3cad82e932b348b1fca433f62089795cdfe82804a3976ff686c767ecbf467|child=task-1-stage0-v12.ps1|child_source_bytes=23048|child_source_sha256=6bff8750bd359adf0b21cf127720a58a20b2e0a02f4954fd1792a4683b5c6a9d|argv=-NoLogo,-NoProfile,-NonInteractive,-File,<repo>\.worktrees\wave0-model-contract\.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery\task-1-stage0-v12.ps1|child_bytes=218|child_sha256=14e0399a8b118beabc96a9fb9d95f119a08d04f8b051c9adbbcc388392afabc1|parent_bytes=325|parent_sha256=d330bed4652d52295271040cda4402978eef125b694998566bd97949f22c17a9|files=2|starts=0'
```

The plan-pinned expected values are child bytes `218`, child SHA-256
`14e0399a8b118beabc96a9fb9d95f119a08d04f8b051c9adbbcc388392afabc1`,
parent bytes `325`, and parent SHA-256
`d330bed4652d52295271040cda4402978eef125b694998566bd97949f22c17a9`.
...
```powershell
& '.\.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery\task-1-stage0-preflight-controller-v12.ps1'
```
...
Both lines end in LF. Any controller uncertainty before child start is `ENTRYV12_PREFLIGHT_CONTROLLER_UNPROVABLE / NO_GO`. Any child start followed by timeout, disconnect, incomplete capture, exit, stdout, stderr, identity, or side-effect difference is `ENTRYV12_STAGE0_UNPROVABLE / NO_GO`. Preserve everything and stop.

- [ ] **Step 3: Run the final read-only closure once**

Require exact design/plan commits and blobs; two-file v12 identities and
...
no unexpected object; v12 controller invocations one; v12 child starts one;
retries zero; and every prohibited action zero.

```powershell
$RepositoryRoot='<repo>\.worktrees\wave0-model-contract'
$CanonicalRoot='<repo>'
$Workspace=Join-Path $RepositoryRoot '.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery'
Set-Location -LiteralPath $RepositoryRoot
$Head=git rev-parse HEAD;$PlanPath='docs/superpowers/plans/2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery.md';$DesignPath='docs/superpowers/specs/2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery-design.md'
if($Head-notmatch'\A[0-9a-f]{40}\z'-or(git rev-list --parents -n 1 HEAD)-cne"$Head 15d34231f7f65f1be6f08eea5946eefd2b8949ba"-or(git rev-parse "HEAD:$DesignPath")-cne'545fe2d74e3209902f1733c180295d89b671ae86'-or(git hash-object -- $PlanPath)-cne(git rev-parse "HEAD:$PlanPath")){throw 'closure lineage/blob rejected'}
if((git show -s --format='%an|%ae|%cn|%ce|%s' HEAD)-cne'kuotunyu|61350295+kuotunyu@users.noreply.github.com|kuotunyu|61350295+kuotunyu@users.noreply.github.com|docs: plan A11 Stage 0 preflight expression recovery'){throw 'closure commit identity rejected'}
if((git rev-list --parents -n 1 15d34231f7f65f1be6f08eea5946eefd2b8949ba)-cne'15d34231f7f65f1be6f08eea5946eefd2b8949ba 30735e992d8670991beb0c2882b8f225ff89d978'-or(git show -s --format='%an|%ae|%cn|%ce|%s' 15d34231f7f65f1be6f08eea5946eefd2b8949ba)-cne'kuotunyu|61350295+kuotunyu@users.noreply.github.com|kuotunyu|61350295+kuotunyu@users.noreply.github.com|docs: design A11 Stage 0 preflight expression recovery'){throw 'closure design identity rejected'}
$PlanChanged=@(git diff-tree --no-commit-id --name-only -r HEAD);$DesignChanged=@(git diff-tree --no-commit-id --name-only -r 15d34231f7f65f1be6f08eea5946eefd2b8949ba);if($PlanChanged.Count-ne1-or$PlanChanged[0]-cne$PlanPath-or$DesignChanged.Count-ne1-or$DesignChanged[0]-cne$DesignPath){throw 'closure changed path rejected'}
...
$Shadows=@('<workspace>\.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery','<repo>\.worktrees\wave0-model-contract\CC_github部隊\vision-active-learning-loop\.worktrees\wave0-model-contract\.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery','<repo>\CC_github部隊\vision-active-learning-loop\.worktrees\wave0-model-contract\.superpowers\sdd\2026-08-30-val-wave0-a11-stage0-preflight-expression-recovery');foreach($Path in $Shadows){if(Test-Path -LiteralPath $Path){throw 'closure shadow rejected'}}
